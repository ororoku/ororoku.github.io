...
### Windows 10で英語キーボードを使う
2022/10/24

以下の手順で使用可能
*「時刻と言語」より「日本語（Windowsの表示言語）」->「オプション」ボタンをクリック
*「ハードウェア キーボード レイアウト」の項目で「レイアウトを変更する」をクリック
*「ハードウェア キーボード レイアウトの変更」ウィンドウが表示されますので「英語キーボード（101/102 キー）」を選択して「OK」をクリック
* PCを再起動

### クリップボードの履歴をONにして連続コピペ
「クリップボードの設定」から「クリップボードの履歴」をONにしておけば、コピーする（Ctrl + C） + ペーストする（Windowsキー + V）で連続ペーストできる。
Ctrl　＋　Vは既存通り、直前コピーした内容をペースト。

### コマンドファイルを用意して文字数カウントする
2022/10/05

Wordを使っても文字数カウントはできるが、Wordが無い場合は以下のようにコマンドファイルを用意して文字数カウントすることもできる。

メモ帳などで以下のコマンドを入力し、「名前を付けて保存」からファイル名を「count.cmd」にして「C:\Users\<ユーザー名>」フォルダに保存
```
@echo off 
...
文字数をカウントしたい文章をクリップボードにコピーした状態で「Windowsキー+Rキー」の同時押しで「ファイル名を指定して実行」を呼び出し、「count」と入力してEnterキーを押せばコマンドプロンプトが開いて文字数をカウントしてくれる。
...
| アプリケーション | 該当なし |
 ...
 _ と ¥ は出せない。キーマップの変更が必要になるが、試していない。

## Errors

### Windows10でDocumentsフォルダが見えなくなる

Explorerの左側には表示されているが、PCフォルダを開いても見えない。PowerShellでlsしても見えない。
パスを打ち込んでcdすれば入れる。

解決。フォルダ自体が隠しファイルになっていた。
右クリック -> プロパティから「隠しファイル」のチェックを外す