...
### Windows上でLinuxを使う
Windows 10では、WSL(Windows Subsystem for Linux)を有効化することでLinuxを利用することが可能である。
その際、以下の様にWindows-Linux間のファイル共有が可能である。
* Linux->Windows : WindowsのCドライブがLinuxの`/mnt/c`にマウントされている。`/mnt/c`にアクセスすることで、Windows上のファイルにアクセス可能。
* Windows->Linux : `\\wsl$\[ディストリビューション名]`でLinux上のファイルにアクセス可能。Ubuntuを使っている場合は、`\\wsl$\Ubuntu`でアクセスする。

## Errors

## References
* 「新しいLinuxの教科書」
* Windows Subsystem for Linux に関するドキュメント <https://docs.microsoft.com/ja-jp/windows/wsl/>
* [【Qiita】Windows 10でLinuxを使う(WSL2)](https://qiita.com/whim0321/items/ed76b490daaec152dc69)
