...
デスクトップパソコンのグラフィックボードには「マザーボードに搭載されているもの」と「CPUに内蔵されているもの」がある。

マザーボードに搭載されているものには、NVIDIAのGeforce「RTX」「GTX」シリーズやAMD 「Radeon」シリーズなどがある。
グラフィックボード自体に映像出力端子も合わせて存在し、交換やアップグレードも可能である。

CPUにグラフィックスの機能が含まれている製品があり「CPU内蔵グラフィックス」と呼ばれる。
CPU内蔵グラフィックスを使用する場合は、マザーボードにある映像端子から映像出力できることが多い。
